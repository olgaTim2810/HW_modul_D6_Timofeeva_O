...
def my_job():#рассылка постов (на подписанные категории) за неделю
    today = datetime.datetime.now()
    last_week = today - datetime.timedelta(days=7)
    posts = Post.objects.filter(dateCreation__gte=last_week)
 #   categories = set(posts.values_list('postCategory', flat=True))  #получим список строк без словарей
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            my_job,
            trigger=CronTrigger(day_of_week="fri", hour="18", minute="00"),  # отправка списка статей каждую пятницу в 18:00
